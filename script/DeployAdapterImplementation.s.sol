...
/// regular slots 0/1. Adapter8004 appends two mappings directly at slots 2-3. New mappings
...
contract DeployAdapterImplementationScript is Script {
    ...
    string internal constant SIG_PRIMARY_COUNTERFACTUAL_AGENT_SET =
        "WalletUBISet(address,bytes32,address,uint256,uint8,address)";
    ...
    function run() external returns (address proxy, address implementation, bytes memory upgradeCalldata) {
        ...
        address liveRegistry = address(Adapter8004(payable(proxy)).identityRegistry());
        ...
        implementation = address(new Adapter8004(liveRegistry));
        vm.stopBroadcast();
        ...
        require(
            address(Adapter8004(payable(implementation)).identityRegistry()) == liveRegistry,
