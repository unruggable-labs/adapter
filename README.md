...
- Under `ACCOUNT` (value `5`), authority is the bound address itself, plus any address it has granted a wallet-wide delegate.xyz delegation. There is no holder, owner, or admin route in: a large token balance grants nothing, an optional `owner()` on the bound contract grants nothing, and the adapter admin grants nothing. The direct path, `msg.sender == boundAddress`, makes zero external authority calls and probes neither `ownerOf`, `owner()`, nor either `balanceOf` shape. The delegate path reads the delegate.xyz registry live and is revocable, so unlike the immutable binding it can be withdrawn at any time. The bound address itself never loses authority; this is the permanent-controller model.
...
- The value-7 `hasRole(bytes32,address)` probe is a `STATICCALL` and accepts only a successful, exactly 32-byte canonical `true` (word `1`). A reverted probe, wrong-length response, zero word, or non-canonical boolean word grants nobody. The response is decoded as a raw integer so malformed booleans return `false` rather than causing a decoding revert. Role membership is read on every call, so revoking the role removes authority in the same transaction.
...
- None of the three account-level standards is part of the single-owner token set, so none gets the ownerless-collection window. `CONTRACT_OWNABLE` and `ACCOUNT` each have a delegate.xyz route; `CONTRACT_ADMIN` does not, because a role is a membership predicate with no single delegator to name. The `ACCOUNT` route accepts a wallet-wide delegation granted by the bound address, read live and revocable. Because it is revocable it confers no permanent control, so the immutable binding is not a constraint the delegation can outlive. A wallet-wide `delegateAll` covers this identity even when it was granted before the binding existed, and that is the defined meaning of a wallet-wide delegation rather than a surprise: it authorizes the delegate across the bound address's present and future rights, this identity included. The bound address narrows the set at any time by revoking or scoping the delegation.
...
Two properties follow from reading ownership live and treating a reverting `ownerOf` as ownerless, and both are rules of the single-owner binding type rather than defects. First, a burned single-owner id is ownerless, so the collection may bind or re-bind it and drive its counterfactual writes; this is the same collection-only window described above, and it is deliberate. Second, because the single-owner authority check reads `ownerOf` unwrapped, `isController` and every authority-gated call **revert** when the bound token's `ownerOf` reverts — a burned id being the ordinary case. A caller that needs a total predicate should treat `isController` as able to revert for single-owner bindings and wrap it accordingly; only the unknown-agent case is guaranteed to return `false`. A binding type with a different lifecycle — one where a burned id stays permanently bound to its last owner, say — would be a separate standard with its own rules, not a change to this one.
