...
contract AttestationProjectionTest is Test {
    ...
    bytes32 internal ubid = keccak256("some counterfactual identity");
    ...
    /// @dev Rule 5, state projection: the latest live attestation per (attester, ubid, type) is
        ...
        returns (bool has, bytes memory data)
    {
        for (uint256 i; i < evs.length; ++i) {
            Ev storage e = evs[i];
            if (e.isRevoke || e.actor != attester || e.ubid != target || e.attType != attType) continue;
            if (isLive(e.id)) {
                has = true;
                data = e.data;
            }
        ...
        returns (address[] memory)
    {
        ...
        for (uint256 i; i < evs.length; ++i) {
            Ev storage e = evs[i];
            if (e.isRevoke || e.ubid != target || e.attType != attType) continue;
    ...
    function testFixtureVectors() public {
        ...
        bytes32 fxUbid = 0x8493ab3adb4f5e8753ee3fe05e377bffe213753e1b4155035fec1705d94615f9;

        vm.prank(alice);
        fx.confirmAdditionalAccount(fxUbid);
        drain();
        assertEq(lastId(), 0x7fde72c738899c71442073381b50194c322b2ea08732ae9b3ea121e57979b58d, "vector 1");

        vm.prank(alice);
        fx.attest(tConfirm, fxUbid, bytes32(uint256(1)), "");
        drain();
        assertEq(lastId(), 0x579f37809a02f3f7fafa7ae9169874cb38d0bd965cc0474d6cafee7fa71c666d, "vector 2");

        vm.prank(alice);
        fx.attest(tRating, fxUbid, bytes32(0), hex"57");
        drain();
    ...
    function testCollapseDuplicateEmitsAreOneStatement() public {
        ...
        (bool has,) = currentState(alice, ubid, tRating);
        assertFalse(has);
    }

    function testVariantSeparatesWithinOneBlock() public {
        vm.startPrank(alice);
        adapter.attest(tReview, ubid, bytes32(0), "solid");
        adapter.attest(tReview, ubid, bytes32(uint256(1)), "solid");
        vm.stopPrank();
        drain();
    ...
    function testRevokeBeforeAttestIsInertThenAttestLands() public {
        bytes32 id = idOf(address(adapter), alice, ubid, tConfirm, bytes32(0), "");
        vm.startPrank(alice);
        adapter.revoke(id);
        adapter.confirmAdditionalAccount(ubid);
        vm.stopPrank();
    ...
    function testReactivationAcrossBlocksIsANewStatement() public {
        ...
        (bool has,) = currentState(alice, ubid, tConfirm);
        assertTrue(has);
    }
    ...
    function testStateResurrection() public {
        vm.prank(alice);
        adapter.attest(tRating, ubid, bytes32(0), hex"32"); // 50
        drain();
        bytes32 fifty = lastId();
        roll(block.number + 1);
        vm.prank(alice);
        adapter.attest(tRating, ubid, bytes32(0), hex"50"); // 80
        drain();
        bytes32 eighty = lastId();

        (bool has, bytes memory data) = currentState(alice, ubid, tRating);
        assertTrue(has);
        ...
        drain();
        (has, data) = currentState(alice, ubid, tRating);
        assertTrue(has);
        ...
        drain();
        (has,) = currentState(alice, ubid, tRating);
    ...
    function testReEmissionMakesAStatementCurrentAgain() public {
        vm.startPrank(alice);
        adapter.attest(tRating, ubid, bytes32(0), hex"32"); // 50
        adapter.attest(tRating, ubid, bytes32(0), hex"3c"); // 60
        adapter.attest(tRating, ubid, bytes32(0), hex"32"); // 50 again, same id as the first
        vm.stopPrank();
        drain();
        (bool has, bytes memory data) = currentState(alice, ubid, tRating);
        assertTrue(has);
    ...
    function testUnstarVersusRevokeAreDifferentOperations() public {
        ...
        vm.prank(alice);
        adapter.attest(tStar, ubid, bytes32(0), hex"01");
        roll(block.number + 1);
        vm.prank(alice);
        adapter.attest(tStar, ubid, bytes32(0), hex"01");
        roll(block.number + 1);
        vm.prank(alice);
        adapter.attest(tStar, ubid, bytes32(0), hex"00");
        drain();
        (bool has, bytes memory data) = currentState(alice, ubid, tStar);
        assertTrue(has);
        assertEq(uint8(data[0]), 0, "attesting zero is a position: unstarred");
        assertEq(starCount(ubid), 0);
        ...
        drain();
        (has, data) = currentState(bob, ubid, tStar);
        assertTrue(has);
        assertEq(uint8(data[0]), 1, "revocation resurfaces the older live star");
        assertEq(starCount(ubid), 1, "bob still stars; alice holds an unstarred position");
    }
    ...
    function testRatingAggregationExcludesInvalidPayloads() public {
        ...
        adapter.attest(tRating, ubid, bytes32(0), hex"50"); // 80
        vm.prank(bob);
        adapter.attest(tRating, ubid, bytes32(0), hex"64"); // 100
        vm.prank(carol);
        adapter.attest(tRating, ubid, bytes32(0), hex"96"); // 150, invalid at read
        drain();
        assertEq(ratingAverage(ubid, new address[](0)), 90, "the invalid rating never enters the average");
    }
    ...
    function testRatingAggregationExcludesDependentAttesters() public {
        ...
        adapter.attest(tRating, ubid, bytes32(0), hex"3c"); // 60
        vm.prank(bob);
        adapter.attest(tRating, ubid, bytes32(0), hex"64"); // 100, but bob is the subject's controller
        drain();
        address[] memory dependents = new address[](1);
        dependents[0] = bob;
        assertEq(ratingAverage(ubid, dependents), 60, "self-review is excluded at aggregation");
        assertEq(ratingAverage(ubid, new address[](0)), 80, "the exclusion is the consumer's choice, applied here");
    }
    ...
    function testStarCountCountsLatestLivePositions() public {
        ...
        adapter.attest(tStar, ubid, bytes32(0), hex"00");
        drain();
        assertEq(starCount(ubid), 2);

        roll(block.number + 1);
        vm.prank(carol);
        adapter.attest(tStar, ubid, bytes32(0), hex"01");
        drain();
        assertEq(starCount(ubid), 3);
    }
}
