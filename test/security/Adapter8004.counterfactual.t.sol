...
contract CounterfactualSecurityTest is Test {
    ...
    function testCounterfactualReclaimEmitsRepeatedEventsAndChainDoesNotDedupe() external {
        ...
        bytes32 topic = keccak256("CounterfactualAgentURISet(bytes32,address,address,bytes,string)");
        bytes32 expectedHash = keccak256(
            abi.encode(
                adapter.interoperableAddress(address(adapter)),
                address(token721),
                abi.encodePacked(uint8(0), uint256(1))
            )
        );
        bytes32 expectedTokenContract = bytes32(uint256(uint160(address(token721))));
        bytes32 expectedEmitter = bytes32(uint256(uint160(alice)));
        uint256 matches;
        for (uint256 i; i < logs.length; ++i) {
            if (logs[i].topics.length > 0 && logs[i].topics[0] == topic) {
                assertEq(logs[i].topics[1], expectedHash);
                assertEq(logs[i].topics[2], expectedTokenContract);
                assertEq(logs[i].topics[3], expectedEmitter);
                // Decode the non-indexed payload (bytes identifier, string newURI) and confirm the
                // canonical identifier rides on every event.
                (bytes memory identifier,) = abi.decode(logs[i].data, (bytes, string));
                assertEq(identifier, abi.encodePacked(uint8(0), uint256(1)), "identifier rides on every event");
                ++matches;
            }
        }
    ...
    function testTopic0DiscriminatesSchemaWithoutAPayloadVersion() external {
        ...
        assertEq(
            logs[0].topics[0],
            keccak256("CounterfactualAgentURISet(bytes32,address,address,bytes,string)"),
