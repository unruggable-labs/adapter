...
contract CounterfactualSecurityTest is Test {
    ...
    function testAllCounterfactualSettersHaveNoRegistryOrAdapterSideEffects() external {
        ...
        vm.startPrank(alice);
        adapter.counterfactualSetAgentURI(IERC8217.TokenStandard.ERC721, address(token721), 1, "ipfs://u");
    ...
    function testCounterfactualSetAgentWalletRequiresNoSignatureAndCannotMutateRegistry() external {
        ...
        vm.prank(alice);
        adapter.counterfactualSetAgentWallet(IERC8217.TokenStandard.ERC721, address(token721), 1, newWallet);
    ...
    function testCounterfactualSurfaceRefusesNonController() external {
        ...
        adapter.counterfactualRegister(IERC8217.TokenStandard.ERC721, address(token721), 1, "u");

        vm.expectRevert(abi.encodeWithSelector(Adapter8004.NotController.selector, eve, type(uint256).max));
        adapter.counterfactualSetAgentURI(IERC8217.TokenStandard.ERC721, address(token721), 1, "u");
        ...
        vm.expectRevert(abi.encodeWithSelector(Adapter8004.NotController.selector, eve, type(uint256).max));
        adapter.counterfactualSetMetadataBatch(IERC8217.TokenStandard.ERC721, address(token721), 1, empty);
    ...
    function testCounterfactualReclaimEmitsRepeatedEventsAndChainDoesNotDedupe() external {
        ...
        vm.startPrank(alice);
        adapter.counterfactualSetAgentURI(IERC8217.TokenStandard.ERC721, address(token721), 1, "ipfs://1");
        adapter.counterfactualSetAgentURI(IERC8217.TokenStandard.ERC721, address(token721), 1, "ipfs://2");
        adapter.counterfactualSetAgentURI(IERC8217.TokenStandard.ERC721, address(token721), 1, "ipfs://3");
        vm.stopPrank();
        ...
        for (uint256 i; i < logs.length; ++i) {
            if (logs[i].topics.length > 0 && logs[i].topics[0] == topic) {
                ...
                assertEq(
                    standard, uint8(IERC8217.TokenStandard.ERC721), "standard must ride on every counterfactual event"
                );
                ++matches;
            }
        }
    ...
    function testTopic0DiscriminatesSchemaWithoutAPayloadVersion() external {
        ...
        assertEq(
            logs[0].topics[0],
            keccak256("CounterfactualAgentURISet(bytes32,address,uint256,uint8,string,address)"),
            "topic0 is the keccak of the current signature"
        );
        // Every superseded shape must fail to match the emitted topic.
        assertTrue(
            logs[0].topics[0] != keccak256("CounterfactualAgentURISet(bytes32,address,uint256,string,address)"),
            "original shape, before extraData and before the standard"
        );
        assertTrue(
            logs[0].topics[0] != keccak256("CounterfactualAgentURISet(bytes32,address,uint256,bytes32,string,address)"),
            "extraData shape, before the standard"
        );
        assertTrue(
            logs[0].topics[0]
                != keccak256("CounterfactualAgentURISet(bytes32,address,uint256,bytes32,uint8,string,address)"),
            "extraData plus standard shape, superseded at 0.0.17"
        );
        assertTrue(
            logs[0].topics[0]
    ...
    function testCounterfactualSetAgentURIReentryRevertsWithReentrancyGuardReentrantCall() external {
        ...
        adapter.counterfactualSetAgentURI(IERC8217.TokenStandard.ERC721, address(mal), 1, "ipfs://outer");
    }
