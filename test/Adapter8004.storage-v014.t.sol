...
abstract contract Adapter8004LiveBaseline is Initializable, OwnableUpgradeable, UUPSUpgradeable, ReentrancyGuard {
    ...
    mapping(uint256 agentId => IERC8217.Binding binding) private _bindings;
    ...
    function seedLiveBinding(uint256 agentId, IERC8217.Standard standard, address boundAddress, uint256 tokenId)
        ...
        onlyOwner
    {
        _bindings[agentId] = IERC8217.Binding({standard: standard, boundAddress: boundAddress, tokenId: tokenId});
    }

    function bindingOf(uint256 agentId) external view returns (IERC8217.Binding memory) {
        return _bindings[agentId];
    }

    function isController(uint256 agentId, address account) external view returns (bool) {
        IERC8217.Binding memory binding = _bindings[agentId];
